...
* Then the pixel effect for brightening based on brighten.c - https://o365coloradoedu-my.sharepoint.com/shared?listurl=%2Fpersonal%2Fsiewerts%5Fcolorado%5Fedu%2FDocuments&viewid=352a79ce%2Ddcf4%2D42ac%2D896f%2D903d069f746d&csf=1&FolderCTID=0x0120000AC3DE8CB97B0647B9FAC11DCE7EAC00&id=%2Fpersonal%2Fsiewerts%5Fcolorado%5Fedu%2FDocuments%2FESEE%2DWeb%2DResources%2FRTES%2D5623%2DCU%2Fcode%2Fc%2Dbrighten%2Fbrighten%2Ec&parent=%2Fpersonal%2Fsiewerts%5Fcolorado%5Fedu%2FDocuments%2FESEE%2DWeb%2DResources%2FRTES%2D5623%2DCU%2Fcode%2Fc%2Dbrighten
                                            - RTES_5623/RTES-ECEE-5623-mcgaffin/c-brighten/brighten.c
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
// Brighten parameters
static const double alpha = 1.35;
static const unsigned char beta = 45;
...
static void dump_ppm(const void *p, int size, unsigned int tag, struct timespec *time)
{
    ...
    snprintf(ppm_dumpname, sizeof(ppm_dumpname), "frames/test%04d.ppm", tag);
...
void yuv2rgb_brighten(int y, int u, int v, unsigned char *r, unsigned char *g, unsigned char *b)
{
   int r1, g1, b1;
...
static unsigned char rgb_buffer[HRES * VRES * 3];
...
static void process_image(const void *p, int size)
{
    ...
    for (int i = 0; i < size; i += 4) {
        ...
        yuv2rgb_brighten(y_temp, u_temp, v_temp, &rgb_buffer[rgb_idx], &rgb_buffer[rgb_idx+1], &rgb_buffer[rgb_idx+2]);
        yuv2rgb_brighten(y2_temp, u_temp, v_temp, &rgb_buffer[rgb_idx+3], &rgb_buffer[rgb_idx+4], &rgb_buffer[rgb_idx+5]);
        rgb_idx += 6;
    }
#ifdef DUMP_FRAMES
    if (framecnt > -1) {
        syslog(LOG_INFO, "frame %d: Processed YUYV to Brightened RGB size %d", framecnt, size);
        dump_ppm(rgb_buffer, sizeof(rgb_buffer), framecnt, &frame_time);
    } else {
    ...
    if (framecnt > -1 && ffmpeg_pipe != NULL) {
        ...
        size_t written = fwrite(rgb_buffer, 1, sizeof(rgb_buffer), ffmpeg_pipe);

        if (written < sizeof(rgb_buffer)) {
            syslog(LOG_ERR, "frame %d: Pipe write mismatch or failure", framecnt);
        } else {
            syslog(LOG_INFO, "frame %d: Streamed brightened frame to MP4 pipe", framecnt);
        }
    } else {
        syslog(LOG_INFO, "frame %d: warm-up ignored", framecnt);
    }
#endif
}
...
{
    char ffmpeg_cmd[] = "ffmpeg -y -f rawvideo -pixel_format rgb24 -video_size 640x480 -framerate 30 -i - -c:v libx264 -pix_fmt yuv420p piped_output.mp4";
