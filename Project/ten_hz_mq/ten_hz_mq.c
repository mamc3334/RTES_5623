...
#define FRAME_COUNT 181 //TODO: change to 1801
#define NUM_BUFFERS 5 // TODO: reduce to 3? 
#define RING_SIZE 10 // should be plenty big
#define FRAME_BYTES (HRES * VRES * 2)
#define MQ_MAX_MESSAGES 50

#define WB_MQ_NAME  "/wb_mq"
...
static void init_posix_mq(void)
{
    struct mq_attr attr;
    attr.mq_flags = 0;
    ...
    // Unlink stale queue
    mq_unlink(WB_MQ_NAME);

    // Open queue - non-blocking
    wb_mq = mq_open(WB_MQ_NAME, O_CREAT | O_RDWR | O_NONBLOCK, 0666, &attr);
    if (wb_mq == (mqd_t)-1) {
        syslog(LOG_ALERT, "Failed to create wb_mq: %s", strerror(errno));
        errno_exit("MQ_OPEN");
    }
}
...
int main(int argc, char **argv)
{
    ...
    openlog("ten_hz", LOG_PID|LOG_CONS, LOG_USER);
    ...
    syslog(LOG_INFO, "10HZ synchronome for digital stopwatch started at %s", time);
    ...
    syslog(LOG_INFO, "TEN_HZ cleanup completed successfully.");
...
static void *sequencer_thread(void *arg)
{
    ...
    while(capture)
    {
        ...
        // Spawn threads
        // if(count % ACQ_F == 0) //50hz - hardware max 30hz
        // int mod = count % PROCESS_F;
        if(count % ACQ_F == 0) //30hz
        {
            sem_post(&sem_acq);
        }
...
static void *process_thread(void *arg)
{
    ...
    if (!cur_frame || !best_frame || !last_frame) {
    ...
    while(capture)
    {
        ...
        {
            pthread_mutex_lock(&in_rb->lock);
            // //skip 1 frames - likely to be old second
            while(size > 2)
            {
        ...
        if (mq_send(wb_mq, (const char *)&best_frame, sizeof(uint8_t *), 0) < 0) {
            if (errno == EAGAIN) {
                syslog(LOG_WARNING, "[PRO] WB Queue full");
            } else {
                syslog(LOG_ERR, "[PRO] mq_send to WB error: %s", strerror(errno));
            }
            errno_exit("MQ_SEND");
        } 
...
static void *writer_thread(void *arg)
{
    ...
    while(capture)
    {
        ...
        // {
        //     syslog(LOG_ALERT, "[WB] semaphore released with no frame to write");
        ...
        ssize_t bytes_read;
        while((bytes_read = mq_receive(wb_mq, (char *)&local_frame, sizeof(uint8_t *), NULL)) > 0)
        {
            int rgb_idx = 0;
        ...
        if (bytes_read < 0) {
            if (errno != EAGAIN) {
                syslog(LOG_ERR, "[WB] mq_receive error: %s", strerror(errno));
                errno_exit("MQ_RECIEVE");
            }
        }
    }
