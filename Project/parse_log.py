...
def parse_log_and_plot(file_path):
    ...
    acq_ideal   = np.median(acq_periods)                 # data-driven nominal
    ...
    pro_times    = np.array(pro_start_times)
    pro_periods  = np.diff(pro_times)
    pro_ideal    = np.median(pro_periods)
    pro_jitter   = pro_periods - pro_ideal
    pro_expected = pro_times[0] + np.arange(len(pro_times)) * pro_ideal
    pro_drift    = pro_times - pro_expected
    ...
    fig2, (ax2a, ax2b) = plt.subplots(2, 1, figsize=(9, 7))
    ...
    ax4a.set_title(f'ACQ Drift vs. Perfect {acq_ideal:.1f} ms Clock – {tag}')
    ...
    ax4b.set_title(f'PRO Drift vs. Perfect {pro_ideal:.1f} ms Clock – {tag}')
