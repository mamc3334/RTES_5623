...
static long get_score(uint8_t *prev, uint8_t *cur)
{
    ...
    // syslog(LOG_DEBUG, "SCORE: %ld", score);
...
int main(int argc, char **argv)
{
    ...
    syslog(LOG_INFO, "1HZ synchronome for analog clock started at %s", time);
...
static void *sequencer_thread(void *arg)
{
    ...
    while(capture)
    {
        ...
        //TODO: look into pit_sig - https://o365coloradoedu-my.sharepoint.com/:u:/r/personal/siewerts_colorado_edu/Documents/ESEE-Web-Resources/RTES-5623-CU/code/POSIX-Examples-Updated/clock_pitsig_test.c?csf=1&web=1&e=uJ5DjT
...
static void *process_thread(void *arg)
{
    ...
    while(capture)
    {
        ...
        //copy size so new frames coming in after processing start dont count
        pthread_mutex_lock(&in_rb->lock);
        int size = in_rb->size;
        pthread_mutex_unlock(&in_rb->lock);
